...
const styles = css`
  ...
  position: relative;
  &[data-overflow][debug-overflow-error],
  [data-overflow][debug-overflow-error] {
  ...
  &[data-overflow][debug-overflow-error]:after,
  [data-overflow][debug-overflow-error]:after {
...
// if already defined don't do anything but if it's a new version of textfit upgrade
if (!customElements.get('text-fit')) { 
  customElements.define('text-fit', TextFit);
} else if (!document.createElement('text-fit') instanceof TextFit) {
  customElements.upgrade('text-fit', TextFit);
}
